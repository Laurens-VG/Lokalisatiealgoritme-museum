...
# Import Information
filename = cf.path_gopro + "calibration_W.mp4"
...
def ImageCollect(filename, n_boards):
    ...
    if status == True:
        ...
        while current_frame < total_frames:
            ...
            if k == 32:
                collected_images += 1
                cv2.imwrite(cf.path_calibration_images + 'Calibration_Image' + str(collected_images) + '.png', image)
...
# The distortion parameter are saved into a numpy file (calibration_W_data.npz).  The total
...
def ImageProcessing(n_boards, board_w, board_h, board_dim):
    ...
    for i in range(1, n_boards + 1):
        ...
        image = cv2.imread(cf.path_calibration_images + 'Calibration_Image' + str(i) + '.png')

        # Converting to grayscale
        grey_image = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
        # cv2.imshow("gray checkerboard", grey_image)
        # cv2.waitKey(0)
        # cv2.destroyWindow("gray checkerboard")
    ...
    for i in range(1, n_boards + 1):
        ...
        image = cv2.imread(cf.path_calibration_images + 'Calibration_Image' + str(i) + '.png')
        undistort_image(image, intrinsic_matrix, distCoeff)


    cv2.destroyAllWindows()


def undistort_image(image, intrinsic_matrix, distCoeff):
    res = cv2.undistort(image, intrinsic_matrix, distCoeff, None)
    cv2.imshow('Undisorted Image', res)
    # cv2.imwrite("Undisorted", res)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
